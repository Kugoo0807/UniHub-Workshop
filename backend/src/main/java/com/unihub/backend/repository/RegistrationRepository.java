...
public interface RegistrationRepository extends JpaRepository<Registration, Long> {
        ...
        java.util.Optional<Registration> findByUserIdAndWorkshopIdAndStatus(Long userId, Long workshopId,
                        String status);
         ...
         */
        @Query(value = "SELECT r FROM Registration r JOIN FETCH r.workshop w WHERE r.user.id = :userId ORDER BY r.createdAt DESC", countQuery = "SELECT COUNT(r) FROM Registration r WHERE r.user.id = :userId")
        Page<Registration> findAllByUserIdWithWorkshop(@Param("userId") Long userId, Pageable pageable);

        // Find PENDING registrations created before the cutoff time (used by
        // cancellation job)
        List<Registration> findByStatusAndCreatedAtBefore(String status, LocalDateTime cutoff);

        /**
         * Bulk-update registration status to CANCELLED for all active registrations of
         * a workshop.
        ...
        /**
         * Count registrations with a specific status that belong to COMPLETED workshops
         * only.
